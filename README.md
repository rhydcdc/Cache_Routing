...
TBT·TTFT·용량이 **433K 부근에서 최저를 찍고 다시 오른다.** 반대 방향의 두 힘이 거기서 교차한다.
...
디코드의 대역폭은 `가중치 70GB + KV` 다. 그래서 규모에 따라 두 극단이 있다.

```
작은 규모   가중치 70GB 가 지배 → BASE 도 우리도 같은 값을 읽는다 → 비율이 100% 에 붙는다
큰 규모     KV 가 지배        → 비율이 「읽는 양의 비율」로 수렴한다
```

∴ 맥락이 길어질수록 비율이 **내려간다.** (부수적으로 — 디코드 중엔 연산 유닛이 84~93% 논다.
이 구조가 성립하는 이유가 여기다. **병목인 대역폭을 아끼고 그 대가를 놀던 연산으로 낸다.**)

#### ↑ 올라가는 힘 — 정제기 몫이 30% 상한이다

클러스터를 **정적으로 쪼갠다** — 서빙 70% / 정제기 30%. 정제기는 일이 있을 때만 가져가지만
**밀려도 30% 가 상한**이다. 맥락이 길어지면 정제 1회에 생성할 양도 같이 커지는데 몫은 그대로라
따라잡지 못하고, 그 사이 원문이 계속 쌓인다.
...
움직이는 건 초과분뿐이고, 그만큼 **「읽는 양의 비율」이 올라간다.**
①이 수렴하려는 목표값 자체가 올라가는 것이다.

#### 두 힘이 만나는 곳

**433K 부근.** 그 아래에서는 ①(가중치 희석)이 이겨 곡선이 내려가고, 위에서는 ②(정제기 포화)가
이겨 다시 올라간다. TBT·TTFT·용량이 한 지점에서 나란히 꺾이는 것이 이 하나의 교차 때문이다.

⚠️ **정적 분할은 모델링 선택이다.** 실제로는 밀릴 때 몫을 늘릴 수 있고 그러면 ②가 약해져
꺾임이 완화된다. 정적으로 잡은 쪽이 우리에게 불리하다.
