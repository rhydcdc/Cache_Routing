# REPREFILL — 자연어를 다시 프리필해 KV 캐시를 만드는 아키텍처

> ### ▶ **[웹으로 보기 — rhydcdc.github.io/RE_PREFILL](https://rhydcdc.github.io/RE_PREFILL/)**
>
...
> 굳이 가까운 비유를 찾자면 MoE 보다 **가상 메모리·페이징**이다 —
> 상주 세트(메인)와 백킹 스토어(세부), 필요할 때 가져오기. 이름이 "포인터"인 이유다.
...
| **3** | **리프리필** | 나뉜 자연어 파일을 각각 프리필한다 | **자연어 → KV** |
| **4** | **보관** | 메인은 **상주**, 세부는 필요할 때 **로드**. 원본 캐시는 버린다 | KV |
| **5** | **질의** | 메인을 읽고 → 자연어로 판단 → 필요한 세부를 읽는다 | **자연어 판단 → KV 로드** |
...
**세부는 상주시키지 않는다.** 읽고 나면 놓아준다 — 로드가 쌓여 상주분이 단일 캐시로 수렴하지 않는다.
대신 같은 세부가 다시 필요하면 다시 프리필한다.

> ⚠️ **KV 바이트만 실측했다.** 지연·처리량은 재지 않았다.
...
|분배 전체(저장)   |78.4%    |97.4%    |96.6%        |**55.2%**  |**51.0%**   |
|표시 비용       |+5.3%    |+14.0%   |+7.5%        |**+31.5%** |**+10.3%**  |
|세부 파일       |23개(53t) |7개(314t) |25개(119t)   |**12개(182t)**|**14개(202t)**|
|**질문 시점 유효**|**44.0%**|**68.2%**|**72.0%**    |**22.8%**  |**26.4%**   |
...
- **지연과 처리량.** 잰 것은 KV 바이트뿐이다. **실제로 얼마나 빨라지는지는 확인하지 않았다**
...
| **디코드의 유휴 연산** | 디코드는 **메모리 대역 바운드**라 연산 유닛이 논다. 정제·분배를 별도 프로세스로 돌리는 대신 **같은 배치에 낮은 우선순위 스트림으로 태우면** 벽시계 비용을 상당 부분 상각할 여지가 있다. ⚠️ 단 **배치가 커지면 연산 바운드로 넘어가 이 여지가 사라지고**, 프리필과 섞으면 TTFT 를 민다. **재지 않았다** |
