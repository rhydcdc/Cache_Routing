...
| | 왜 |
|---|---|
| **임계 전에 안 하는 이유** | 매번 정제·분배하면 비용이 매번 들고, 조금씩 얹으면 흐름 판단을 그때마다 다시 해야 한다 |
| **임계에서 통째로 하는 이유** | 전체를 한 번에 보므로 **새 턴이 앞 턴을 재진술한 것까지** 잡힌다. 그리고 **얹기로 불어난 메인이 리셋된다** |
...
- **필요한 세부를 매번 정확히 짚었다.** 25개 중 1개, 23개 중 1개, 14개 중 2개 — 여섯 번 다 맞혔고
  **필요 없는 세부까지 읽은 적은 없다.** 왜 그 파일인지 근거도 스스로 댔다
